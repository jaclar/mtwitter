...
function Twitter(options) {
  ...
  if (!this.options.application_only){
...
Twitter.prototype.get = function(url, params, callback) {
  ...
  var handle = function (error, data, response) {
...
Twitter.prototype.appAuth = function(callback) {
  var self = this;

  /*
   * Use request plugin to get access to our app's access token.
   * as in stated in https://dev.twitter.com/docs/api/1.1/post/oauth2/token
   */

  request.post({
      url: self.options.authenticate_application_url,
      followAllRedirects: true,
      headers: {
          "User-Agent": 'ntwitter/' + VERSION,
          "Authorization": "Basic " + new Buffer(self.options.consumer_key + ":" + self.options.consumer_secret).toString("base64"),
    ...
    if (!error && response.statusCode == 200) {
      ...
      if (data.hasOwnProperty("token_type") && data.token_type === "bearer") {
        self.options.bearer_token = data.access_token;
...
Twitter.prototype.destroyFavorite = function(id, params, callback) {
  var url = '/favorites/destroy.json?id=' + escape(id);
  this.post(url, params, null, callback);
  return this;
}
